"""
Ported directly from the original Flask app.py.
Symptom -> disease prediction using the trained SVC model + CSV lookups.
Loaded once at import time (Django keeps this in memory per worker, same as Flask did).
"""
import os
import numpy as np
import pandas as pd
...
DATA_DIR = os.path.join(settings.BASE_DIR, 'datasets')
...
class UnknownSymptomError(Exception):
    """Raised when a symptom the user typed isn't in our known symptom set."""
    def __init__(self, bad_symptoms):
        self.bad_symptoms = bad_symptoms
        super().__init__(f"Unknown symptoms: {bad_symptoms}")
...
def get_predicted_value(patient_symptoms):
    ...
    unknown = [s for s in patient_symptoms if s not in symptoms_dict]
    if unknown:
...
def predict_from_text(raw_symptoms_text):
    """
    Takes the comma-separated symptom string exactly like the Flask form did,
    returns a clean dict ready for JSON response.
    """
    user_symptoms = [s.strip() for s in raw_symptoms_text.split(',')]
    user_symptoms = [s.strip("[]' ") for s in user_symptoms]
    user_symptoms = [s for s in user_symptoms if s]  # drop empties

    predicted_disease = get_predicted_value(user_symptoms)
    dis_des, precautions_raw, medications, rec_diet, workout = helper(predicted_disease)

    my_precautions = []
    if len(precautions_raw) > 0:
        for i in precautions_raw[0]:
            if isinstance(i, str) and i.strip():
                my_precautions.append(i)

    def flatten(raw_list):
        """CSV stores these as stringified Python lists — unpack them."""
        result = []
